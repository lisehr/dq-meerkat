...
public class NullValuesPercentage extends DependentProfileStatistic {
  ...
  protected void dependencyCalculationWithRecordList(RecordList rl) {
    if (super.getMetricPos(nullValP) - 2 <= super.getMetricPos(numrows)) super.getRefProf().getStatistic(numrows).calculation(rl, null);
    if (super.getMetricPos(nullValP) - 1 <= super.getMetricPos(nullVal)) super.getRefProf().getStatistic(nullVal).calculation(rl, null);

  }

  @Override
  protected void dependencyCalculationWithNumericList(List<Number> list) throws NoSuchMethodException {
    if (super.getMetricPos(nullValP) - 2 <= super.getMetricPos(numrows)) super.getRefProf().getStatistic(numrows).calculationNumeric(list, null);
    if (super.getMetricPos(nullValP) - 1 <= super.getMetricPos(nullVal)) super.getRefProf().getStatistic(nullVal).calculationNumeric(list, null);
  }
  ...
  protected void dependencyCheck() {
    ...
    ProfileStatistic nullV = super.getRefProf().getStatistic(nullVal);
    if (nullV == null) {
