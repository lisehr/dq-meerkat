...
import static dqm.jku.dqmeerkat.quality.profilingstatistics.StatisticTitle.mad;
...
public class MedianAbsoluteDeviation extends DependentProfileStatistic {
	...
	protected void dependencyCalculationWithNumericList(List<Number> list) throws NoSuchMethodException {
		...
		if (super.getMetricPos(sd) - 1 <= super.getMetricPos(med)) super.getRefProf().getStatistic(med).calculationNumeric(list, null);
	}
	...
	protected void dependencyCalculationWithRecordList(RecordList rl) {
		...
		if (super.getMetricPos(sd) - 1 <= super.getMetricPos(med)) super.getRefProf().getStatistic(med).calculation(rl, null);
	}
