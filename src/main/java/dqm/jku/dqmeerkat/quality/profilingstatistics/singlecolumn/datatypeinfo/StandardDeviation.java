...
public class StandardDeviation extends DependentProfileStatistic {
    ...
    private Object addValue(Object current, Object toAdd, Object avg) {
        ...
        if (a.getDataType().equals(Long.class))
            return (long) current + powerN(((((Number) toAdd).longValue() - ((Number) avg).longValue())), 2);
            ...
            return (int) current + (int) Math.pow((double) (((String) toAdd).length() - ((String) avg).length()), 2);
        else
            return (numberToBigInteger(current).add(numberToBigInteger(toAdd).subtract(numberToBigInteger(avg)).pow(2)));
    }
    ...
    private Object performAveraging(Object sum) {
        ...
        Attribute a = (Attribute) super.getRefElem();
        if (a.getDataType().equals(Long.class))
            return Math.sqrt((((long) sum / ((long) super.getRefProf().getStatistic(numrows).getValue() - 1))));
        else if (a.getDataType().equals(Double.class))
