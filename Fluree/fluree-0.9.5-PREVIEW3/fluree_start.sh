...
FLUREE_SERVER=fluree_server.jar
...
if [[ "$JAVA_X" ]]; then
    ...
    if [[ "$JAVA_VERSION" < "1.8" ]]; then
        ...
        exit -1
    elif [[ "$JAVA_VERSION" > "1.9" ]]; then
        echo "FlureeDB is tested on Java 8 (v1.8). You are running experimental support with version: $JAVA_VERSION"
    else
...
if ! [ -f $FLUREE_SERVER ]; then
    echo "FlureeDL.jar file not found. Looking for $FLUREE_SERVER. Exiting."
    exit 1
fi

echo "FlureeDL starting with properties file: $PROPERTIES_FILE"
echo "FlureeDL starting with java options: $XMS $XMX $JAVA_OPTS"

exec $JAVA_X -server $XMX $XMS $JAVA_OPTS -Dfdb.properties.file=$PROPERTIES_FILE -Dfdb.log.ansi -jar $FLUREE_SERVER
